...
!      integer, parameter :: x_division = 2, y_division = 2 
      ...
      real(kind=rk),    parameter :: bound = 0.0
      contains
!################################################################################
!-------------------------------------------------------------------------------- 
...
!--------------------------------------------------------------------------------
      ...
      real(kind=rk) :: x, y, x1, x2, x3, x4, y1, y2, y3, y4, 
     $        m1, m2, m3, m4, a1, a2, a3, a4

!      integer :: ie1, je1, im_fine1, jm_fine1

      real(kind=rk), dimension(im_coarse, jm_coarse) :: 
     $          alon_coarse1, alat_coarse1
      real(kind=rk), dimension(im,jm) :: alon_fine1, alat_fine1
...
!--------------------------------------------------------------------------------
       ...
       do j = 1, jm
          do i = 1, im
             ...
             i1 = int(real(i-1)/x_division + is - 0.5)
             j1 = int(real(j-1)/y_division + js - 0.5)
...
!--------------------------------------------------------------------------------
         ...
         real(kind=rk), intent(in) :: x,y, x1,x2,x3,x4, y1,y2,y3,y4, 
     $                                     m1,m2,m3,m4
         real(kind=rk), intent(out) :: w1, w2, w3, w4
         integer, intent(out) :: flag

!--------------------------------------------------------------------------------

         real(kind=rk) :: t, s, a1, a2, a3, a4, b1, b2, b3, b4, A, B, C, 
     $           tot_w, tot_m
